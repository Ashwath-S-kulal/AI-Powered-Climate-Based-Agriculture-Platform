    ...
    <footer className="bg-gray-900 text-gray-300 border-t border-gray-800">
      <div className="max-w-screen mx-auto px-3 sm:px-6 md:px-20 py-12">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {quickLinks.map(({ label, to }) => (
                <li key={to}>
                  <NavLink to={to} className="text-sm text-gray-400 hover:text-white transition-colors duration-150">
                    {label}
                  </NavLink>
                </li>
              ))}
        ...
        <div className="mt-10 pt-6 border-t border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-gray-500">© 2025 SmartAgri. All rights reserved.</p>
          <p className="text-xs text-gray-500">Built for climate-resilient agriculture</p>
        </div>
      </div>
    </footer>
  );
