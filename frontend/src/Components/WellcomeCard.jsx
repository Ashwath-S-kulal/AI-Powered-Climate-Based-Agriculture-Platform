...
import { Sprout, CloudSun, Microscope, Lightbulb, Info, TriangleAlert, CalendarCheck, ChevronLeft, ChevronRight, LandPlot } from "lucide-react";
...
const quickActions = [
  { to: "/croplibrary/croplist", label: "Crops", icon: Sprout, color: "bg-emerald-100 text-emerald-700", description: "Crop varieties & growing steps" },
  { to: "/weather", label: "Weather", icon: CloudSun, color: "bg-sky-100 text-sky-700", description: "Current weather & forecasts" },
  { to: "/disease", label: "Diagnose", icon: Microscope, color: "bg-amber-100 text-amber-700", description: "Scan crops for diseases" },
  { to: "/soil", label: "Soil", icon: LandPlot, color: "bg-amber-100 text-amber-700", description: "Analyse your Soil" },
  { to: "/croprecomnder", label: "Recommend", icon: Lightbulb, color: "bg-violet-100 text-violet-700", description: "Crop suggestions for your area" },
  { to: "/croplibrary/cropinfo", label: "Crop Info", icon: Info, color: "bg-blue-100 text-blue-700", description: "Detailed crop knowledge base" },
  { to: "/cropriskcalculater", label: "Risk", icon: TriangleAlert, color: "bg-rose-100 text-rose-700", description: "Calculate crop risks" },
];
...
export default function WelcomeHero() {
  ...
  return (
    <div>
      {/* Hero Slider */}
      <div className="relative w- h-[350px] sm:h-[400px] overflow-hidden bg-gray-900 mx-2 md:mx-8 my-3 rounded-3xl">
        <div
            ...
            {quickActions.map((item, index) => {
              ...
              return (
                <NavLink key={index} to={item.to} className="group">
                  <div className="flex flex-col items-center p-3 rounded-xl hover:bg-gray-50 transition-all duration-150 text-center cursor-pointer border border-transparent hover:border-gray-200">
                    ...
                    <p className="mt-2 text-xs font-semibold text-gray-700 group-hover:text-gray-900">{item.label}</p>
                    <p className="mt-0.5 text-[10px] text-gray-400 leading-tight hidden sm:block">{item.description}</p>
                  </div>
                </NavLink>
              );
            })}
