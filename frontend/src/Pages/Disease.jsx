            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="p-6 border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-zinc-800">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div
                    className={`flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-xl cursor-pointer transition-colors min-h-[130px] ${uploadedImageFile
                      ...
                      : "border-gray-300 dark:border-zinc-700 hover:border-emerald-400 hover:bg-gray-50 dark:hover:bg-zinc-900"
                      }`}
                    onClick={() => !uploadedImageFile && fileInputRef.current?.click()}
                  >
                    {uploadedImageFile ? (
                      <div className="flex flex-col items-center gap-2">
                        <img src={URL.createObjectURL(uploadedImageFile)} alt="preview" className="w-16 h-16 object-cover rounded-lg border border-gray-200 dark:border-zinc-700" />
                        <p className="text-xs font-medium text-gray-700 dark:text-zinc-300 truncate max-w-[110px]">{uploadedImageFile.name}</p>
                      ...
                      <>
                        <Upload size={22} className="text-gray-300 dark:text-zinc-600 mb-2" />
                        <p className="text-xs font-semibold text-gray-600 dark:text-zinc-300">Upload File</p>
                        <p className="text-[10px] text-gray-400 dark:text-zinc-600 mt-0.5">PNG, JPG, JPEG</p>
                      </>
                    )}
                  ...
                  <div className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-300 dark:border-zinc-700 hover:border-emerald-400 hover:bg-gray-50 dark:hover:bg-zinc-900 rounded-xl cursor-pointer transition-colors min-h-[130px]">
                    {isCameraActive ? (
                      ...
                      <div className="flex flex-col items-center gap-2">
                        <img src={URL.createObjectURL(capturedImageFile)} alt="captured" className="w-16 h-16 object-cover rounded-lg border border-gray-200 dark:border-zinc-700" />
                        <p className="text-xs font-medium text-gray-700 dark:text-zinc-300">Captured Image</p>
                      ...
                      <button type="button" onClick={() => { setUploadedImageFile(null); setIsCameraActive(true); }} className="flex flex-col items-center gap-2 text-gray-500 dark:text-zinc-400 hover:text-emerald-600 transition-colors w-full h-full justify-center">
                        <Camera size={22} className="text-gray-300 dark:text-zinc-600" />
                        <p className="text-xs font-semibold">Use Camera</p>
                        <p className="text-[10px] text-gray-400 dark:text-zinc-600">Scan in real-time</p>
                      </button>
                    )}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {loading ? (
              ...
              <div className="p-6 bg-gray-50 dark:bg-black/40 transition-colors duration-200">
                ...
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-14 gap-3">
                    ...
                    <p className="text-sm text-gray-500 dark:text-zinc-400">Running neural diagnosis...</p>
                  </div>
                ) : !result ? (
                  <div className="flex flex-col items-center justify-center py-14 text-center">
                    <div className="w-12 h-12 bg-gray-100 dark:bg-zinc-800 rounded-xl flex items-center justify-center mb-3">
                      <Microscope size={20} className="text-gray-400 dark:text-zinc-600" />
                    </div>
                    <p className="text-sm text-gray-400 dark:text-zinc-500">Upload or snap a leaf photo,<br />then click Analyze to view results.</p>
                  </div>
                ) : result.invalid ? (
                  <div className="flex flex-col items-center justify-center py-14 text-center">
                    <div className="w-12 h-12 bg-red-50 dark:bg-red-950/30 rounded-xl flex items-center justify-center mb-3">
                      <ImageOff size={20} className="text-red-400 dark:text-red-500" />
                    </div>
                    <p className="text-sm font-bold text-red-600">Invalid Image</p>
                    <p className="text-xs text-gray-500 dark:text-zinc-500 mt-1">Please provide a clearer, closer photo of the crop leaf.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
        ...
        {loading ? (
          <div className="mt-6 space-y-4 animate-pulse">
            <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 p-4 rounded-xl space-y-3">
              ...
              <div className="h-10 w-full bg-emerald-100 dark:bg-emerald-900/50 rounded"></div>
              <div className="space-y-2">
                <div className="h-8 w-full bg-emerald-100 dark:bg-emerald-900/50 rounded"></div>
                <div className="h-8 w-full bg-emerald-100 dark:bg-emerald-900/50 rounded"></div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 p-4 rounded-xl">
              ...
              <ul className="space-y-2">
                {result.advice.steps.map((step, i) => (
                  <li key={i} className="text-xs text-emerald-800 dark:text-emerald-200 bg-emerald-100/50 dark:bg-emerald-900/30 p-2 rounded-lg">
                    {typeof step === 'string' ? (
                      <p className="flex items-center gap-1.5">
                        <CheckCircle2 size={12} className="text-emerald-600 dark:text-emerald-400" /> {step}
                      </p>
                    ) : (
                      <>
                        <p className="font-bold flex items-center gap-1.5">
                          <CheckCircle2 size={12} className="text-emerald-600 dark:text-emerald-400" /> {step.action}
                        </p>
                        <p className="ml-5 text-[10px] text-emerald-700/80 dark:text-emerald-400/80 mt-0.5">
          ...
          <NavLink to="/croplibrary/diseasedata" className="group">
            <div className="bg-white dark:bg-[#09090b] border border-gray-200 dark:border-zinc-800 rounded-xl p-5 hover:shadow-sm hover:border-gray-300 dark:hover:border-zinc-700 transition-all">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-emerald-600 transition-colors">Disease Database Catalog</h3>
              </div>
              <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed">Explore comprehensive information on plant conditions, descriptions, and mitigation strategies.</p>
            </div>
          </NavLink>
          ...
          <NavLink to="/disease/diseasesuppliment" className="group">
            <div className="bg-white dark:bg-[#09090b] border border-gray-200 dark:border-zinc-800 rounded-xl p-5 hover:shadow-sm hover:border-gray-300 dark:hover:border-zinc-700 transition-all">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-emerald-600 transition-colors">Treatment & Products</h3>
              </div>
              <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed">View treatment solutions, organic bio-fertilizers, and crop supplements from agronomists.</p>
            </div>
          </NavLink>
        </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-zinc-400 border border-gray-300 dark:border-zinc-700 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-900 transition-colors"
              >
                <RefreshCw size={12} /> <span className="hidden md:inline">Refresh</span>
              </button>
              <button
                onClick={() => handleClearHistory(currentUser._id)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-600 border border-red-200 dark:border-red-900/50 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
              >
                <Trash2 size={12} /> <span className="hidden md:inline">Clear</span> All
              </button>
            </div>
          </div>

          <div className="space-y-3 px-3 md:px-5 py-2">
            {history.map((h) => (
              <div key={h._id} className="border border-gray-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-[#09090b] hover:border-emerald-200 dark:hover:border-emerald-900 transition-all">
                <div className="flex items-center justify-between p-2 md:p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-900">
                  <div className="flex items-center gap-3">
                    <div className={`hidden md:inline p-2 rounded-lg ${h.advice ? 'bg-emerald-50 dark:bg-emerald-950/30' : 'bg-gray-100 dark:bg-zinc-800'}`}>
                      <CheckCircle2 size={18} className="hidden md:inline text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-gray-900 dark:text-white">{h.prediction}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      {expandedId === h._id ? "Hide" : "View"}<span className="hidden md:inline">Advise</span>
                    </button>
                ...
                {expandedId === h._id && h.advice && (
                  <div className="px-4 pb-4 pt-0 border-t border-gray-100 dark:border-zinc-800 bg-emerald-50/30 dark:bg-emerald-950/20 animate-in slide-in-from-top-2">
                    ...
                    <p className="text-xs text-gray-600 dark:text-zinc-400 leading-relaxed mb-3">
                      {h.advice.explanation}
                    </p>
                    <div className="grid gap-2">
                      {h.advice.steps.map((step, idx) => (
                        <div
                          key={idx}
                          className="flex gap-2 p-2 bg-white dark:bg-zinc-900 rounded-lg border border-emerald-100 dark:border-emerald-900"
                        >
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-500 dark:text-zinc-500">
                              {step.description} •
                              <span className="uppercase text-[9px] font-semibold ml-1">
                                {step.method}
                              </span>
                            </p>
                          </div>
