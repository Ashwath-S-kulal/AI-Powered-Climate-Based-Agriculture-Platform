      ...
      <div className="max-w-7xl mx-auto px-6 pt-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          ...
          <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
            ...
            <span className="font-semibold text-slate-900">{users.length} Active Accounts</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map((user) => (
              <div key={user._id} className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-center gap-4 mb-6">
                  <img src={user.profilePicture || "/default-avatar.png"} className="w-16 h-16 rounded-full object-cover border-2 border-emerald-100" />
                  <div>
                    <h3 className="font-bold text-slate-900">{user.username}</h3>
                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium ${user.isAdmin ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600'}`}>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-100">
                  <button onClick={() => handleRoleChange(user._id)} className="flex-1 text-xs font-bold uppercase py-2 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition">
                    Toggle Role
                  </button>
                  <button onClick={() => handleDeleteUser(user._id)} className="px-3 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition">
                    <FaTrashAlt size={14} />
                  </button>
                </div>
              </div>
