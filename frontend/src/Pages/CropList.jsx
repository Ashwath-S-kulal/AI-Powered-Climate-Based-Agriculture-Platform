...
export default function CropsList() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      {/* Hero Header Area (Subtle) */}
      <div className="w-full bg-white border-b border-slate-200 pt-8 pb-12 ">
      ...
      <div className="px-4 w-full max-w-7xl mx-auto relative -top-6 z-20">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-8 flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="relative flex-1 w-full" ref={searchInputRef}>
            <div className="flex items-center bg-slate-50 rounded-xl px-4 py-2.5 gap-3 border border-slate-200 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
              <FiSearch className="text-slate-400 text-lg flex-shrink-0" />
              <input
                type="text"
                placeholder="Search crop name..."
                className="w-full outline-none text-slate-800 placeholder:text-slate-400 bg-transparent text-sm"
                value={search}
                onChange={(e) => {
            ...
            {showSuggestions && searchSuggestions.length > 0 && (
              <div className="absolute w-full mt-2 z-30 max-h-60 overflow-y-auto bg-white rounded-xl shadow-lg border border-slate-100 py-1">
                {searchSuggestions.map((name, i) => (
                  ...
                  >
                    <GiTreeGrowth className="text-emerald-500" />
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            ...
            <button
              onClick={reset}
              className="flex items-center justify-center text-xs gap-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 py-2.5 px-4 rounded-xl transition-colors font-medium hover:text-slate-900 w-full sm:w-auto"
            >
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20 bg-white border border-slate-200 rounded-2xl shadow-sm">
            <GiTreeGrowth className="mx-auto text-4xl text-slate-300 mb-3" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
          {filtered.map((c) => (
            <div
              ...
              className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm cursor-pointer hover:border-slate-300 hover:shadow-md transition-all duration-300 group flex flex-col"
            >
              <div className="h-44 overflow-hidden bg-slate-100 relative border-b border-slate-100">
                ...
                {/* Subtle overlay gradient to ensure contrast if we put text over image, but here we don't */}
              </div>

              <div className="p-5 flex-1 flex flex-col">
                <h2 className="text-lg font-bold text-slate-900 mb-1 leading-tight group-hover:text-emerald-700 transition-colors">
                  {c.name}
                </h2>
                
                <div className="mt-auto pt-4 flex justify-between items-center text-sm">
                  <span className="flex items-center px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600 border border-slate-200/60">
                    {c.type}
                  </span>
      ...
      >
        ...
        <div
          className={`absolute mt-10 bottom-0 left-0 right-0 w-full lg:max-w-4xl max-h-[85vh] mx-auto bg-white md:rounded-t-3xl shadow-2xl flex flex-col transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) ${
            isDetailViewOpen ? "translate-y-0" : "translate-y-full"
          }`}
        >
          {selectedCrop && (
            <>
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white md:rounded-t-3xl z-10">
                <h2 className="text-xl font-bold text-slate-900">{selectedCrop.name} Guide</h2>
                <button
                  onClick={() => setSelectedCropNum(null)}
                  className="p-2 bg-slate-50 text-slate-500 rounded-full hover:bg-slate-100 hover:text-slate-900 transition-colors border border-slate-200"
                >
