...
const successfulFarmers = [
  {
    ...
    fullStoryUrl: "https://en.wikipedia.org/wiki/Hukumchand_Patidar"
  },
  {
...
export default function CropLibrary() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await fetch('https://api.rss2json.com/v1/api.json?rss_url=https://news.google.com/rss/search?q=agriculture');
        const data = await response.json();
        if (data.status === 'ok') {
          // Grabbing the top 3 news items
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <NavLink to={"/croplibrary/marketprices"} className="block group">
              <div className="p-6 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm hover:border-emerald-300 dark:hover:border-emerald-500 hover:shadow-md transition-all h-full flex flex-col relative overflow-hidden">
                ...
                <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-50 mb-2">Market Price Dashboard</h3>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6 flex-grow leading-relaxed">
                  Access real-time APMC wholesale commodity rates and track market price trends across India.
                </p>
                <div className="mt-auto inline-flex items-center text-sm font-semibold text-emerald-600 dark:text-emerald-400">
                  Check Prices <ArrowRight className="ml-1.5 w-4 h-4 transition-transform group-hover:translate-x-1" />
                </div>
              </div>
            </NavLink>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {news.map((item, idx) => (
                ...
                >
                  ...
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-50 mb-3 line-clamp-3 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors leading-relaxed">
                    {item.title}
                  </h3>
                  <div className="mt-auto inline-flex items-center text-xs font-medium text-zinc-500 dark:text-zinc-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
